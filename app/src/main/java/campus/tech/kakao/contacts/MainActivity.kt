    ...
    class MainActivity : AppCompatActivity() {
        ...
        override fun onCreate(savedInstanceState: Bundle?) {
            ...
            cancle.setOnClickListener {
                contactManager.showToast(this@MainActivity, "취소되었습니다.")
            }

            save.setOnClickListener {
                if(contactManager.checkIsFilled(name.getText().toString())&&contactManager.checkIsFilled(phoneNumber.getText().toString()))
                {
                    Log.d("textField", name.getText().toString())
                    contactManager.showToast(this@MainActivity, "저장되었습니다.")
                } else {
                    contactManager.showToast(this@MainActivity, "이름과 번호 입력은 필수입니다.")
                }
            }
        }
    }
